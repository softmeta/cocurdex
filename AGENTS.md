...
不要自行启动 `pnpm --filter @cocurdex/desktop dev`。如果用户的本地开发进程已经在运行，应优先复用该进程自行验证，包括通过 CDP 检查界面与运行时状态、查看日志、操作界面，以及在改动需要时触发热更新或重载应用。只有本地开发进程未运行或无法访问时，才告知用户自行启动并验证桌面应用。
