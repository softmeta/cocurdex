...
不要自行启动 `pnpm --filter @cocurdex/desktop dev`。用户本地开发进程未运行时，告知用户自行启动。不要为确认外观或交互去复用进程、连 CDP、截图或在应用里点来点去。

## UI / UX 验证

外观、布局、间距、对齐、交互、动画、空态等 UI/UX 改动，由用户在本机应用里自行验证。agent 做完实现和「修改后校验」后，用一两句话说明改了哪里、建议看哪个界面即可结束，不要再开浏览器/CDP 巡检、截图对比或端到端点选。

- 用户未明确要求时，不要为「确认长得对 / 点起来对」消耗额外回合或 token。
- 运行时报错、控制台异常、可复现的功能故障：仍按代码与日志排查；不要把「看起来对不对」当成必须由 agent 验收的项。
- 只有用户明确要求「帮我看界面 / 连 CDP / 截个图」时，才做界面侧检查。
