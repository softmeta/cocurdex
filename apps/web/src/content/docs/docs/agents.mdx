...
| `opencode` | OpenCode | External `opencode` CLI | Plan mode, streaming; steering not supported |
| `grok-build` | Grok Build | External `grok` CLI (ACP transport) | Plan mode, steering over ACP; native write only |
| `pi` | Pi | Built-in | Read-oriented default; no separate permission modes |
...
| Lightweight chat-style turn without native workspace write | `pi` (Pi) |
...
| `claude-auto` | elevated — offered when the installed Claude CLI supports auto mode |
| `claude-bypass-permissions` | dangerous |

When the collaboration mode is `plan`, the session uses `claude-plan` (plan only, no edits or commands). Switching back to `default` restores `claude-default`.
...
No separate permission-mode list; write mode is **read-only**.
...
An **agent role** is a saved preset: agent + provider/model + permission mode + collaboration and reasoning axes, plus optional instructions and skills. Manage roles under **Settings → Agent roles** (or save the current session's setup as a role from the session UI), and list them from the CLI:

```bash
cocurdex role list
```

Roles can bind [workflow](/docs/workflows/) planner / implementer / reviewer slots via `--planner-role`, `--implementer-role`, and `--reviewer-role`. A role binds only when its permission profile matches what the workflow role requires: planner and reviewer need a read-only profile, implementer needs workspace write, and dangerous permission modes cannot bind at all.
