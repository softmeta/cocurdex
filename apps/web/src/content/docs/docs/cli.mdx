...
| `--version` / `-v` / `-V` / `version` | Print CLI version |
...
`--root` defaults to the current working directory for project scope. `cocurdex skill` is accepted as an alias. See [Skills](/docs/skills/).
...
Managed git worktrees give sessions an isolated checkout of a workspace repository.
...
`worktree settings` with no flags prints the current settings; pass `--root` and/or `--fetch`/`--no-fetch` to update them. `--root` sets the directory where Cocurdex creates managed worktrees (empty clears it back to the default location); `--fetch` controls whether the repository is fetched before creating a worktree.
...
Saved agent presets (agent + provider/model + permission + reasoning axes) that can also bind workflow roles. Manage them in the desktop UI; list them from the CLI:
...
Default role agents when flags are omitted: planner `codex`, implementer `grok-build`, reviewer `codex`. A `--<role>-role` flag binds a saved [agent role](#agent-roles) instead of an ad-hoc agent/model pair; the role's permission profile must match what the workflow role requires.
