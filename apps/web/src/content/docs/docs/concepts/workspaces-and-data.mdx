...
| `rootPaths` | Absolute paths on disk; the first entry is the primary root |
...
| Agent roles, worktree settings, session records | Daemon + `cocurdex.sqlite` |
| Managed git worktrees | Created by the daemon under the configured worktree root |
