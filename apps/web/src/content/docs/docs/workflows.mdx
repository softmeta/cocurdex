...
Optional model overrides: `--planner-model`, `--implementer-model`, `--reviewer-model`. To bind a saved [agent role](/docs/agents/#agent-roles) instead of an ad-hoc agent/model pair, use `--planner-role`, `--implementer-role`, or `--reviewer-role`. `--definition <id>` selects a non-default workflow definition.
...
3. Watch steps transition — the built-in definition runs **plan → approve_plan (gate) → implement → validate → review**, and review can request changes that loop back to implement.
4. When a **gate** needs you, **approve** or **reject** with optional reason (`--step` defaults to the `approve_plan` gate).
5. **Cancel** if the goal changed or the run is stuck. A rejected plan gate ends the run as cancelled; a failed validation or blocked review ends it as blocked.
