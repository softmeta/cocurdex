  ...
  return (
    <InputGroup className={cn("w-auto", className)}>
      <ComboboxPrimitive.Input
        // Keep the intrinsic size small: inside a shrink-to-fit popup a
        // default-sized input (size=20) becomes the widest child and inflates
        // the whole popup. The field still stretches via flex-1.
        render={<InputGroupInput disabled={disabled} size={8} />}
        {...props}
      />
...
function ComboboxItem({
  ...
  return (
    <ComboboxPrimitive.Item
      ...
      className={cn(
        "relative flex w-full cursor-default items-center gap-2 rounded-control py-1 pe-8 ps-1.5 text-sm outline-hidden select-none data-highlighted:bg-accent data-highlighted:text-accent-foreground not-data-[variant=destructive]:data-highlighted:**:text-accent-foreground data-disabled:pointer-events-none data-disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
        className,
      )}
      {...props}
    >
